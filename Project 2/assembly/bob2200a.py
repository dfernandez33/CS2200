...
"""bob2200a.py: A definition of the BOB-2200a architecture."""
__author__ = "There's a lot of people on this one honestly"
...
class bne(Instruction):
    __RE_BNE = re.compile(r'^\s*(?P<RX>\$\w+?)\s*,\s*(?P<RY>\$\w+?)\s*,\s*(?P<Offset>\S+?)\s*$')
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        match = cls.__RE_BNE.match(operands)
        if match is None:
            raise RuntimeError("Operands '{}' are in an incorrect format.".format(operands.strip()))
